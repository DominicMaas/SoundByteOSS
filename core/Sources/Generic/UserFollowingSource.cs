...
{
    public class UserFollowingSource : ISource
    {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            switch (Service)
            {
                ...
                case ServiceTypes.SoundCloudV2:
                    var scSource = Activator.CreateInstance<SoundCloudUserFollowingSource>();
                    scSource.UserId = UserId;
                    return await scSource.GetItemsAsync(count, token, cancellationToken);

                case ServiceTypes.YouTube:
                    var ytSource = Activator.CreateInstance<YouTubeUserFollowingSource>();
